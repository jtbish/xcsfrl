...
def deletion(pop):
    ...
    pop_size = _calc_num_micros(pop)
    num_to_delete = max(0, (pop_size - max_pop_size))
    for _ in range(num_to_delete):
        _delete_single_microclfr(pop)
    assert _calc_num_macros(pop) >= _MIN_NUM_MACROS
    assert _calc_num_micros(pop) <= max_pop_size


def _calc_num_micros(pop):
    return sum([clfr.numerosity for clfr in pop])


def _calc_num_macros(pop):
    return len(pop)
...
def _delete_single_microclfr(pop):
    ...
    vote_sum = 0
    for (vote, clfr) in zip(votes, pop):
        vote_sum += vote
...
def _deletion_vote(clfr, pop):
    ...
    avg_fitness_in_pop = sum([clfr.fitness
                              for clfr in pop]) / _calc_num_micros(pop)
